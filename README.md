<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,2,5,30&height=140&section=header" />

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Orbitron&weight=900&size=45&duration=4000&pause=1000&color=58A6FF&center=true&vCenter=true&width=700&height=80&lines=Syed+Faraaz+Ahmed" alt="Syed Faraaz Ahmed" />
</p>
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3000&pause=1000&color=8b949e&center=true&vCenter=true&width=700&height=40&lines=AI%2FML+Engineer+%7C+LLMs+%7C+RLHF+%7C+RAG+%7C+Distributed+ML+Systems" alt="Role" />
</p>

<p align="center">
  <a href="https://github.com/faraazzz">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=58A6FF&center=true&vCenter=true&width=650&lines=Building+AI+systems+that+are+reliable%2C+aligned%2C+and+production-ready!" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=faraazzz&label=Profile+Views&color=58a6ff&style=for-the-badge" />
  &nbsp;
  <a href="mailto:syedfaraaazahmed@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-syedfaraaazahmed%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white&labelColor=0d1117" />
  </a>
  &nbsp;
  <a href="https://www.linkedin.com/in/syedfaraaz/">
    <img src="https://img.shields.io/badge/LinkedIn-syedfaraaz-0077B5?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0d1117" />
  </a>
</p>

---

## 🧑‍💻 About Me

```python
class SyedFaraaz:
    name        = "Syed Faraaz Ahmed"
    role        = "AI/ML Engineer"
    location    = "San Jose, CA"
    email       = "syedfaraaazahmed@gmail.com"
    experience  = "5+ years"
    current     = "Scale AI — LLM Evaluation & Synthetic Data Platforms"
    specialties = ["LLMs", "RLHF/RLAIF", "RAG", "Multi-Agent Systems", "Distributed ML"]
    motto       = "Build AI that's reliable, aligned, and production-ready 🚀"
```

- 🔭 Currently building **enterprise-scale LLM evaluation & synthetic data generation** platforms at **Scale AI**
- 🧠 Specialized in **RLHF/RLAIF pipelines**, **multi-agent systems**, and **retrieval-augmented generation**
- ⚡ Processing **1.5M+ prompts/week** across finance, legal, healthcare, and code domains
- ☁️ Cloud-native on **AWS (EKS, EC2, S3)** with distributed GPU training via **PyTorch DDP & Ray**
- 🎯 Proven track record of reducing hallucinations, improving model alignment, and optimizing benchmarking pipelines

---

## 💼 Experience

### Scale AI — AI/ML Engineer *(June 2024 – Present · San Jose, CA)*

- Architected enterprise LLM evaluation platform supporting **12+ foundation models** (LLaMA 3.x, Mistral, Mixtral, Falcon, BERT, RoBERTa)
- Designed distributed evaluation pipelines using **PyTorch Lightning, Hugging Face & Ray** — processing **1.5M+ prompts/week**
- Implemented **RLHF & RLAIF** workflows with human preference modeling, improving alignment by **31%**
- Built multi-agent evaluation systems **(planner, critic, verifier)** for reasoning depth and hallucination risk assessment
- Applied **LoRA/QLoRA** fine-tuning — reduced GPU memory by **60%** with comparable performance to full fine-tuning
- Engineered hybrid **RAG pipelines** (FAISS + LangChain + S3), reducing hallucination rate by **28%**
- Built **adversarial red-teaming** infrastructure for jailbreak detection, prompt injection, and long-context stress testing
- Reduced benchmarking runtime by **42%** via distributed GPU infra on **AWS EKS + Ray**
- Built real-time LLM monitoring dashboards using **Prometheus & Grafana** for hallucination, drift, and bias tracking
- Automated synthetic data curation pipelines (JSONL, PDF, multimodal) — increasing annotation efficiency by **45%**

### Accenture — ML Engineer *(Jan 2020 – July 2023 · India)*

- Built recommendation systems, fraud detection, and campaign optimization models using **XGBoost & LightGBM**
- Deployed deep learning models **(CNNs, LSTMs, Transformers)** for text analytics and customer insights
- Fine-tuned **BERT-based models** for text classification, sentiment analysis, and document processing
- Used **Apache Spark, Hadoop, and Hive** for large-scale distributed data pipelines
- Designed A/B testing and performance evaluation frameworks ensuring measurable business impact

---

## 🛠️ Tech Stack

<p align="center">

<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white"/>
<br/><br/>

<img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white"/>
<img src="https://img.shields.io/badge/PyTorch_Lightning-792EE5?style=for-the-badge&logo=lightning&logoColor=white"/>
<img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white"/>
<img src="https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black"/>
<img src="https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/>
<br/><br/>

<img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white"/>
<img src="https://img.shields.io/badge/Ray-028CF0?style=for-the-badge&logo=ray&logoColor=white"/>
<img src="https://img.shields.io/badge/FAISS-0078D4?style=for-the-badge&logoColor=white"/>
<img src="https://img.shields.io/badge/Pinecone-000000?style=for-the-badge&logoColor=white"/>
<br/><br/>

<img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white"/>
<img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
<img src="https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white"/>
<br/><br/>

<img src="https://img.shields.io/badge/MLflow-0194E2?style=for-the-badge&logo=mlflow&logoColor=white"/>
<img src="https://img.shields.io/badge/Prometheus-E6522C?style=for-the-badge&logo=prometheus&logoColor=white"/>
<img src="https://img.shields.io/badge/Grafana-F46800?style=for-the-badge&logo=grafana&logoColor=white"/>
<img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white"/>
<img src="https://img.shields.io/badge/Terraform-7B42BC?style=for-the-badge&logo=terraform&logoColor=white"/>
<br/><br/>

<img src="https://img.shields.io/badge/Apache_Spark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white"/>
<img src="https://img.shields.io/badge/Apache_Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white"/>
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white"/>

</p>

---

## 🚀 Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">

### ☁️ Serverless Medallion Architecture — Crypto Analytics

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/Serverless-Medallion-Architecture-Pipeline-for-Crypto-Analytics-AWS)
![Kafka](https://img.shields.io/badge/-Kafka-231F20?style=flat-square&logo=apache-kafka&logoColor=white)
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)

Real-time crypto analytics on AWS:

```
Kafka → Glue (Bronze/Silver/Gold) → Athena → Redshift → QuickSight
```

- 🥉 Bronze / 🥈 Silver / 🥇 Gold medallion layers
- ⚡ Serverless architecture — zero infra management
- 📊 Live dashboards via QuickSight

    </td>
    <td width="50%" valign="top">

### 🤖 Agentic AI ETL Debugging Pipeline

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/Agentic-AI-ETL-Debugging-Pipeline)
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![Bedrock](https://img.shields.io/badge/-Bedrock-FF9900?style=flat-square&logo=amazonaws&logoColor=white)

Serverless ETL with **Agentic AI debugging**:

- 🧠 Amazon Bedrock for autonomous debugging
- 🔁 Self-healing pipeline on failure
- ☁️ Fully serverless on AWS Lambda + S3

    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">

### 🏦 Real-Time Smart Bank Streaming

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/Real-Time-Smart-Bank-Data-Streaming-Capture_BigData)
![Kafka](https://img.shields.io/badge/-Kafka-231F20?style=flat-square&logo=apache-kafka&logoColor=white)
![Spark](https://img.shields.io/badge/-Spark-E25A1C?style=flat-square&logo=apachespark&logoColor=white)

**Production-grade** banking analytics pipeline:

- ⚡ Real-time transaction ingestion with Confluent Kafka
- 🔒 Fraud detection-ready stream processing
- 📦 High-throughput data at scale with Apache Spark

    </td>
    <td width="50%" valign="top">

### 🔬 Chip Defect Detection & Classification

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/Chip-Defect-Detection-CV)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![YOLOv8](https://img.shields.io/badge/-YOLOv8-00BFFF?style=flat-square)
![Flask](https://img.shields.io/badge/-Flask-000000?style=flat-square&logo=flask&logoColor=white)

**Computer vision** for semiconductor QC:

- 🔍 **YOLOv8** detects 6 PCB defect types
- 🧠 **Custom CNN** classifies 9 wafer failure patterns
- 🌐 Flask web app with image upload & results UI
- ☁️ AWS S3 integration for data & model outputs

    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">

### 🤖 AI Gmail Summarization Pipeline

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/AI-Gmail-Summarization-pipeline)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Ollama](https://img.shields.io/badge/-Ollama-000000?style=flat-square)

End-to-end **AI email automation**:

```
Gmail → dlt → Ollama (LLM) → BigQuery → Slack
```

- 🧠 Summarizes with a **local LLM** (no cloud API cost)
- 📊 Stores structured data in BigQuery
- 📲 Pushes summaries to Slack

    </td>
    <td width="50%" valign="top">

### 🏠 Real Estate Value Estimation

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/RealEstate-Value-Estimation-on-Zillow)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/-Scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)

**ML regression model** on Zillow Prize data:

- 🏡 Predicts property valuation errors with ML
- 🔍 Feature engineering on property data
- 📈 Multiple regression models benchmarked (RMSE, R²)

    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">

### 🚕 NYC Taxi Data Mining

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/Data-Mining-Project_NyC-Taxi)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)

**Predictive modeling** on NYC Yellow Taxi 2024:

- 📊 Data mining & EDA on large-scale trip data
- 🔮 Fare and demand forecasting models
- 🗺️ Geospatial feature engineering

    </td>
    <td width="50%" valign="top">

### 🎨 Unpaired Image-to-Image Translation

[![Repo](https://img.shields.io/badge/View_on_GitHub-181717?style=flat-square&logo=github)](https://github.com/faraazzz/Unpaired-Image-to-Image-Translation)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)

**Deep learning** cross-domain image translation:

- 🖼️ Implements UNIT framework (no paired training data)
- 🎭 Cross-domain style transfer
- 📐 Evaluated with FID & visual quality metrics

    </td>
  </tr>
</table>

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=faraazzz&theme=tokyonight" height="180"/>
  &nbsp;
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=faraazzz&theme=tokyonight" height="180"/>
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=faraazzz&theme=tokyonight" height="180"/>
  &nbsp;
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=faraazzz&theme=tokyonight&utcOffset=8" height="180"/>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=faraazzz&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=faraazzz&theme=tokyo-night&hide_border=true&area=true" />
</p>

---

## 🎓 Education

**San Jose State University** — M.S. in Data Analytics

---

## 📬 Let's Connect

<p align="center">
  <a href="mailto:syedfaraaazahmed@gmail.com">
    <img src="https://img.shields.io/badge/Email_Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  &nbsp;
  <a href="https://www.linkedin.com/in/syedfaraaz/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  &nbsp;
  <a href="https://github.com/faraazzz">
    <img src="https://img.shields.io/badge/Follow_on_GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

<p align="center">
  <i>Open to collaborations on AI/ML, LLM systems, and data engineering projects!</i><br/>
  <i>Feel free to reach out — always happy to connect with fellow builders 🚀</i>
</p>

---

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:16213e,50:1a1a2e,100:0D1117&height=120&section=footer" />
